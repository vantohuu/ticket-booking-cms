...
export const updateUserProfile = (data) => api.put(`${BASE_URL}${USER_SERVICE}/accounts/manager/profile`, data)

export const getUsersPagination = (page = 0, size = 10, sort = "id,desc", search = "") => {
  const url = `${BASE_URL}${USER_SERVICE}/accounts/customers?page=${page}&size=${size}&sort=${sort}`
  return api.get(url)
}
...
export const searchUsers = (params) => {
  ...
  return api.get(
    `${BASE_URL}${USER_SERVICE}/accounts/customers/search?keyword=${encodeURIComponent(search)}&page=${page}&size=${size}&sort=${sort}`,
  )
}
